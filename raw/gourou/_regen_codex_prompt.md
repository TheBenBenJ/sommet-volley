# Regen Le Gourou — identité verrouillée sur idle_0

Tu as l’image de référence jointe : `assets/gourou/idle_0.png` (Le Gourou / Sommet Volley).

## Mission

Regénère **toutes les autres frames** (PAS `idle_0` — vérité).
Écris dans `raw/gourou/<anim>_<n>.png`. Travaille par **LOTS de 3–4** (ordre ci-dessous).
Après chaque lot : copie vers `raw/gourou/` + contact sheet, puis lot suivant.

## LOCK identité (OBLIGATOIRE)

Même personnage que la référence, pixel-identity :
- grosse tête chibi, corps mince/ascète
...
Quand les 26 sont dans `raw/gourou/` (et `idle_0` intact) :

```bash
python3 tools/cutout.py raw/gourou assets/gourou
```

Corrige `assets/gourou/manifest.json` : `idle: 2`, `walk: 4`, comptes canoniques.
