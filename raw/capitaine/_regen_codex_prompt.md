# Regen Le Capitaine — identité verrouillée sur idle_0

Tu as l’image de référence jointe : `assets/capitaine/idle_0.png` (Le Capitaine / Sommet Volley).

## Mission

Regénère **toutes les autres frames** (PAS `idle_0` — vérité).
Écris dans `raw/capitaine/<anim>_<n>.png`. Travaille par **LOTS de 3–4** (ordre ci-dessous).
Après chaque lot : copie vers `raw/capitaine/` + contact sheet, puis lot suivant.
...
Quand les 26 sont dans `raw/capitaine/` (et `idle_0` intact) :

```bash
python3 tools/cutout.py raw/capitaine assets/capitaine
```

Corrige `assets/capitaine/manifest.json` : `idle: 2`, `walk: 4`, comptes canoniques.
