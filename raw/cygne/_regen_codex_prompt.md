# Regen Le Cygne — identité verrouillée sur idle_face_0

Tu as l’image de référence jointe : `assets/cygne/idle_face_0.png` (Le Cygne / Sommet Volley).

## Mission

Regénère **toutes les autres frames** (PAS `idle_face_0` — vérité).
Écris dans `raw/cygne/<anim>_<n>.png`. Travaille par **LOTS de 3–4** (ordre ci-dessous).
Après chaque lot : copie vers `raw/cygne/` + contact sheet, puis lot suivant.

## LOCK identité (OBLIGATOIRE)

Même personnage que la référence, pixel-identity :
- identité = portrait face joint : jeune technocrate, cheveux noirs slicked-back,
...
### Lot 1 — idle corps (dérivés du portrait)
| fichier | pose |
|---------|------|
...
Quand les 26 sont dans `raw/cygne/` (et `idle_face_0` intact) :

```bash
python3 tools/cutout.py raw/cygne assets/cygne
```

Corrige `assets/cygne/manifest.json` : `idle: 2`, `walk: 4`, comptes canoniques.
**Ne touche PAS** à `idle_face_0.png` (raw ni assets).
